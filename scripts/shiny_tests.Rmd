...
This script includes more wrangling and experimenting that resulted in the shiny app script.
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    #nearest_marker <- EEA_data[which.min(colSums(t(EEA_data[-1]) - event)^2),1]    
    
    #print(nearest_marker)
    # Pick out the point
    #subset(EEA_data, nearest == nearest_marker)
    ...
    leafletProxy("map") %>% clearPopups() %>% 
      ...
      addPopups(
        lng = event$coordinates$lng,
        lat = event$coordinates$lat, "Here are you! Your nearest safe space is ")
  })
  
  zipsInBounds <- reactive({
    if (is.null(input$map_bounds))
      return(EEA_data[FALSE,])
    ...
    subset(EEA_data,
           lat >= latRng[1] & lat <= latRng[2] &
             long >= lngRng[1] & long <= lngRng[2], select = geometry)
  })

...
date: "2021-05-11 updated 2021-05-27"
...
# Saving data
st_write(raw_points, "data/raw/EEA_raw_points.shp")
st_write(raw_polygons, "data/raw/EEA_raw_polygons.shp")
st_write(raw_lines, "data/raw/EEA_raw_lines.shp")
st_write(raw_multipolygons, "data/raw/EEA_raw_multipolygons.shp")
```
...
raw_points <- st_read("data/raw/EEA_raw_points.shp")
raw_polygons <- st_read("data/raw/EEA_raw_polygons.shp")
raw_lines <- st_read("data/raw/EEA_raw_lines.shp")
raw_multipolygons <- st_read("data/raw/EEA_raw_multipolygons.shp")
```
...
(projection is changed for the sake of visualizations)
...
```{r}
# Visualizing multipolygons
plot(st_geometry(world_transform));plot(st_geometry(EEA_rmultipoly),col = "red", add = TRUE)
# Visualizing lines
plot(st_geometry(world_transform));plot(st_geometry(EEA_rline), col = "red", add = TRUE)
```

Neither the lines nor the multipolygons show up when plotted, and when one looks at the dataframes themselves, one can see that the lines lack values in most of the columns we found relevant, and that the multypolugons lack the columns for most of these categories to begin with. Given this (and that we are not sure what these geometries are supposed to represent to begin with), we have elected to exclude them from further analysis. 
...
EEA_safe_osm_filtered$name <- iconv(EEA_safe_osm_filtered$name, from = "UTF-8", to = "UTF-8")
...
unique(EEA_safe_osm_filtered$opnng_h)
# > One immidiate issue identified: Encoding of characters
# Fixing encoding 
# (while the variavle from OSM is called opening_hours, saving and loading the data using read_sf changes it to opnng_h)
EEA_safe_osm_filtered$opnng_h <- iconv(EEA_safe_osm_filtered$opnng_h, from = "UTF-8", to = "UTF-8")

#===> Cleaning $website
unique(EEA_safe_osm_filtered$website)
# > Probably best not to mess with this - there do not seem to be any obvious issues
```
...
st_write(EEA_safe_osm_filtered, "data/EEA_lines.shp")
st_write(EEA_safe_osm_filtered, "data/EEA_multypolygons.shp")
```
